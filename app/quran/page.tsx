...
function VerseMarker({
  ...
  return (
    <span
      ...
      aria-hidden="true"
    >
      <svg viewBox="0 0 40 40" width={dims} height={dims} className="absolute inset-0">
        <path
          d={SEAL_PATH}
      ...
      <span className={`relative ${filled ? "text-visual-teal" : "text-neutral-400"}`}>
...
function octagonVertices(radius: number, rotation = 0): [number, number, number][] {
...
function GeometricLattice({ tone }: { tone: string }) {
  ...
  useFrame((state) => {
    ...
    if (!g) return;
    g.rotation.z += 0.0004;
    g.rotation.x = THREE.MathUtils.lerp(g.rotation.x, state.pointer.y * 0.06, 0.03);
    g.rotation.y = THREE.MathUtils.lerp(g.rotation.y, state.pointer.x * 0.08, 0.03);
  });

  return (
    <group ref={groupRef} position={[0, 0, -2.2]}>
      <Line points={outerStar} color={tone} lineWidth={0.6} transparent opacity={0.16} />
      <Line points={outline} color={tone} lineWidth={0.4} transparent opacity={0.08} />
    </group>
  );
}
...
function DriftingLight() {
  ...
  useFrame((state) => {
    ...
    if (!l) return;
    l.position.x = THREE.MathUtils.lerp(l.position.x, state.pointer.x * 3, 0.05);
    l.position.y = THREE.MathUtils.lerp(l.position.y, state.pointer.y * 2, 0.05);
  });
  return <pointLight ref={light} position={[0, 0, 3]} intensity={2.2} distance={9} color="white" />;
}
...
function GenesisHero() {
  ...
  useLayoutEffect(() => {
    ...
    if (reduced) {
      ...
      gsap.set(".genesis-markers", { opacity: 1, y: 0 });
      gsap.set(".genesis-ui", { opacity: 1, y: 0 });
      return;
    }
    ...
    const ctx = gsap.context(() => {
      ...
      if (starRef.current) {
        ...
        gsap.set(starRef.current, { strokeDasharray: len, strokeDashoffset: len });
      }

      gsap.set(".genesis-verse", { opacity: 0, scale: 1.5, filter: "blur(16px)" });
      gsap.set(".genesis-word-iqra", { opacity: 0, scale: 0.82 });
      gsap.set(".genesis-paper", { opacity: 0 });
      gsap.set(".genesis-markers", { opacity: 0, y: 14 });
      gsap.set(".genesis-ui", { opacity: 0, y: 26 });
      ...
      tl.to(radials, { strokeDashoffset: 0, stagger: 0.11, duration: 1.6, ease: "power1.inOut" }, 1);

      // 2.8 → 4 — the star polygon connects the points.
      if (starRef.current) {
        tl.to(starRef.current, { strokeDashoffset: 0, duration: 1.2, ease: "power1.inOut" }, 2.8);
      }

      // 3.6 → 4.8 — اقرأ emerges from inside the construction.
      tl.to(".genesis-word-iqra", { opacity: 1, scale: 1, duration: 1.2, ease: "power2.out" }, 3.6);

      // 5 → 6.9 — اقرأ dissolves as the full verse takes over.
      tl.to(".genesis-word-iqra", { opacity: 0, scale: 1.1, duration: 1, ease: "power1.in" }, 5);
      tl.to(".genesis-verse", { opacity: 1, scale: 1, filter: "blur(0px)", duration: 1.4, ease: "power2.out" }, 5.5);

      // 6 → 8 — the camera pulls back: construction compresses, paper and
      // ink arrive, marginalia settle in.
      tl.to(".genesis-group", { scale: 0.6, duration: 1.9, ease: "power2.inOut" }, 6);
      tl.to(".genesis-void", { opacity: 0, duration: 1.6 }, 6);
      tl.to(".genesis-paper", { opacity: 1, duration: 1.8 }, 6.2);
      tl.to(".genesis-markers", { opacity: 1, y: 0, duration: 1, stagger: 0.08 }, 7.1);

      // 8 → 10 — UI arrives last, once the scene has fully resolved.
      tl.to(".genesis-ui", { opacity: 1, y: 0, duration: 1.2, ease: "power2.out" }, 8);
    }, trackRef);
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        <div className="genesis-paper absolute inset-0">
          <div
            className="absolute inset-0"
            style={{ backgroundImage: "url('/backgrounds/bg-19.png')", backgroundSize: "cover", backgroundPosition: "center" }}
          />
          <div
            className="absolute inset-0"
            style={{
        ...
        <div className="genesis-group absolute inset-0 flex items-center justify-center">
          <svg
            ...
            aria-hidden="true"
          >
            <circle className="genesis-point" cx="0" cy="0" r="3" fill="var(--color-visual-teal)" />
            {octagon.map((v, i) => (
              <path
                key={i}
            ...
            <path ref={starRef} d={starD} stroke="var(--color-visual-teal)" strokeWidth={1.1} strokeOpacity={0.75} fill="none" />
          </svg>
        ...
        <div className="genesis-verse pointer-events-none absolute inset-x-0 top-1/2 -translate-y-1/2 px-4 text-center">
          <h1
        ...
        <div className="genesis-markers absolute bottom-32 left-1/2 flex -translate-x-1/2 items-center gap-4 md:bottom-40">
          {[1, 2, 3, 4, 5].map((n) => (
            <VerseMarker key={n} index={n} filled={n === 1} size="sm" />
          ))}
        </div>

        <div className="genesis-ui absolute inset-x-0 bottom-10 flex flex-col items-center gap-6 px-6 text-center md:bottom-14">
          ...
          <p className="max-w-md font-thmanyah-text text-body leading-relaxed text-neutral-600">
            مساحة هادئة لتلاوة القرآن وحفظه ومراجعته وتدبّره، خطوة تلو أخرى، بلا استعجال.
          </p>
...
const JOURNEY_STAGES = [
  {
    ...
    description: "الصوت قبل المعنى. تبدأ العلاقة بالقرآن من النطق الصحيح، وترتيل الآية كما نزلت، بلا استعجال.",
  },
  {
    id: "tajwid",
    number: 2,
    ...
    description: "كل حرف له موضعه من الفم والحلق. حين يستقر النطق، يصبح للتلاوة إيقاع لا يتغيّر من قراءة لأخرى.",
  },
  {
    id: "hifz",
    number: 3,
    ...
    description: "تكرار محسوب، لا حفظ عشوائي. الآيات تُبنى فوق بعضها كما تُبنى صفحات المصحف، صفحة إثر صفحة.",
  },
  {
    id: "muraja",
    number: 4,
    ...
    description: "الحفظ بلا مراجعة يتلاشى. هذه هي الحلقة التي تُبقي كل ما سبق حاضرًا، لا ذكرى بعيدة.",
  },
  {
    id: "tadabbur",
    number: 5,
    ...
    description: "في النهاية، القرآن ليس نصًا يُحفظ فحسب، بل معنى يُعاش. هنا تكتمل الرحلة، ثم تبدأ من جديد.",
  },
] as const;
...
const MEMORIZATION_MAP: { surah: string; verses: VerseState[] }[] = [
  { surah: "الفاتحة", verses: Array(7).fill("memorized") },
  {
    ...
    verses: [
      ...
      "memorized", "reviewing", "reviewing", "memorized", "memorized",
      "needs-work", "reviewing", "memorized", "memorized", "reviewing",
      "memorized", "needs-work", "memorized", "reviewing", "memorized",
    ],
  },
  {
    surah: "البقرة ٢١–٤٠",
    verses: [
      "reviewing", "not-started", "not-started", "reviewing", "not-started",
      "needs-work", "not-started", "not-started", "reviewing", "not-started",
...
const TAJWEED_LETTERS = [
  { letter: "ء", name: "الهمزة", makhraj: "أقصى الحلق", description: "تخرج من أقصى الحلق، وهي أبعد الحروف مخرجًا. يُنطق بها بانطلاقة الصوت من الحنجرة قبل استقراره." },
  { letter: "ق", name: "القاف", makhraj: "أقصى اللسان وما يليه من الحنك الأعلى", description: "تلتقي أقصى اللسان بأقصى الحنك، فيتولّد صوت مفخّم مستعلٍ يميّزها عن الكاف القريبة منها مخرجًا." },
  { letter: "ط", name: "الطاء", makhraj: "طرف اللسان وأصول الثنايا العليا", description: "من الحروف المطبقة المستعلية؛ يرتفع وسط اللسان نحو الحنك فيكسبها التفخيم المميز لها عن التاء." },
  { letter: "ع", name: "العين", makhraj: "وسط الحلق", description: "تخرج من وسط الحلق، بين مخرج الهمزة والحاء وبين مخرج الغين والخاء، بصوت مستقر لا احتكاك فيه." },
  { letter: "ل", name: "اللام", makhraj: "حافة اللسان مع ما يحاذيها من الحنك الأعلى", description: "تخرج من حافة اللسان بعد مخرج الضاد، وتمتد من أول حافة اللسان إلى منتهى طرفه تقريبًا." },
  { letter: "ر", name: "الراء", makhraj: "طرف اللسان قريبًا من مخرج النون", description: "تخرج من طرف اللسان مع ما يحاذيه من الحنك الأعلى، وفيها صفة التكرير التي تُضبط فلا تُبالَغ فيها." },
] as const;
...
const DAILY_REVIEW_ITEMS = [
  ...
  { label: "البقرة ١١–٢٠", status: "in-progress" as const, detail: "قيد المراجعة" },
...
const TEACHERS = [
  ...
  { name: "أ. مريم القحطاني", focus: "تحفيظ الأطفال", years: "٩ سنوات", quote: "" },
  { name: "الشيخ عبد الله الفارسي", focus: "التجويد وأحكام التلاوة", years: "١١ سنة", quote: "" },
  { name: "أ. هدى الشمري", focus: "التفسير الميسّر", years: "٧ سنوات", quote: "" },
];

const PROGRESS_MILESTONES = [
  { date: "محرم ١٤٤٦", label: "بداية الرحلة", detail: "حفظ سورة الفاتحة" },
  { date: "صفر ١٤٤٦", label: "١٠ آيات ثابتة", detail: "البقرة ١–١٠ دون خطأ لثلاث مراجعات" },
  { date: "ربيع الأول ١٤٤٦", label: "أول جزء مبدوء", detail: "الانتقال إلى الحفظ اليومي المنتظم" },
  { date: "اليوم", label: "٤٧ يومًا متتاليًا من المراجعة", detail: "دون انقطاع" },
];
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <span className="font-thmanyah-text text-caption text-neutral-500">سورة البقرة</span>
        <span dir="ltr" className="font-thmanyah-text text-micro text-neutral-400">{progress}%</span>
      </div>
      <p className="mt-4 font-thmanyah-display text-h3-sm leading-[2.1] text-neutral-900">
        اللَّهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ{" "}
        <VerseMarker index={255} filled size="sm" className="mx-1 align-middle" />
        {" "}لَا تَأْخُذُهُ سِنَةٌ وَلَا نَوْمٌ
      </p>
      <div className="mt-6 h-[2px] w-full rounded-full bg-neutral-200">
        <div className="h-full rounded-full bg-visual-teal transition-all duration-700" style={{ width: `${progress}%` }} />
      </div>
    </div>
  );
}
...
function JourneySection() {
  ...
  return (
    <section ref={rootRef} dir="rtl" className="relative bg-neutral-0 py-24 md:py-40">
      <ScrollThread progressSource={rootRef} className="right-6 top-0 hidden h-full md:block" />
      <SectionContainer>
        <SectionLede lead="الرحلة" sub="خمس محطات على خط واحد" body="كل محطة تبني على ما قبلها. لا يمكن إتقان التدبر دون تلاوة سليمة، ولا يثبت الحفظ دون مراجعة." />
      </SectionContainer>
      ...
      <div className="relative mt-16 grid grid-cols-1 md:grid-cols-2">
        <div className="sticky top-0 flex h-screen items-center justify-center overflow-hidden">
          ...
          <div className="pointer-events-auto absolute bottom-24 right-6 max-w-xs text-right md:right-12">
            <AnimatePresence mode="wait">
              {active === 0 ? (
                <motion.div key="mushaf" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.4 }}>
                  <MushafPreview />
                </motion.div>
              ) : (
                <motion.div key={stage.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.4 }}>
                  <VerseMarker index={stage.number} filled size="md" />
                  <h3 className="mt-4 font-thmanyah-display text-h2-sm text-neutral-900">{stage.term}</h3>
                  <p className="mt-3 font-thmanyah-text text-body leading-relaxed text-neutral-600">{stage.description}</p>
                </motion.div>
              )}
            </AnimatePresence>
            <div className="mt-6 flex items-center gap-2">
              {JOURNEY_STAGES.map((s, i) => (
                <span
                  ...
                  style={{ backgroundColor: i <= active ? "var(--color-visual-teal)" : "var(--color-neutral-200)" }}
                />
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-24 px-6 md:gap-40 md:px-10">
          {JOURNEY_STAGES.map((s, i) => (
            <div key={s.id} className="journey-step flex min-h-[50vh] flex-col justify-center">
              <span className="font-thmanyah-text text-micro text-neutral-400">{toArabicDigits(i + 1)}</span>
              <h4 className="mt-3 font-thmanyah-display text-h3 text-neutral-900">{s.title}</h4>
              <p className="mt-3 max-w-md font-thmanyah-text text-body text-neutral-600">{s.description}</p>
            </div>
          ))}
        </div>
      </div>
...
function MemorizationSection() {
  const [hovered, setHovered] = useState<{ surah: string; index: number; state: VerseState } | null>(null);

  return (
    <section dir="rtl" className="relative bg-neutral-50 py-24 md:py-32">
      <SectionContainer>
        <SectionLede lead="الحفظ" sub="تراكم، لا شريط تقدّم" body="كل نقطة آية تراكمت مع سابقتها. مرّر المؤشر فوق أي نقطة لترى حالتها." />
      </SectionContainer>
      ...
      <div className="mt-16 flex flex-col gap-16 px-6 md:px-10">
        {MEMORIZATION_MAP.map((group, gi) => {
          ...
          return (
            <div key={group.surah}>
              <p className="font-thmanyah-text text-caption text-neutral-500">{group.surah}</p>
              <div className="mt-4 overflow-x-auto">
                <svg width={width} height={64} viewBox={`0 0 ${width} 64`} className="max-w-full">
                  {group.verses.map((state, i) => {
                    const x = 13 + i * 26;
                    const y = 32 + Math.sin(i * 0.9 + gi) * 14;
                    const isHovered = hovered?.surah === group.surah && hovered.index === i + 1;
                    return (
                      <circle
                        key={i}
                        cx={x}
                        ...
                        onMouseEnter={() => setHovered({ surah: group.surah, index: i + 1, state })}
                        onMouseLeave={() => setHovered(null)}
                        onFocus={() => setHovered({ surah: group.surah, index: i + 1, state })}
        ...
        <div className="flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-neutral-200 pt-8">
          {MEMORIZATION_LEGEND.map((item) => (
            <div key={item.state} className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: STATE_STYLE[item.state].fill }} />
              <span className="font-thmanyah-text text-caption text-neutral-600">{item.label}</span>
            </div>
          ))}
          <div className="min-h-[1.25rem]">
            <AnimatePresence mode="wait">
              {hovered && (
                ...
                >
                  {hovered.surah} — آية {toArabicDigits(hovered.index)} — {STATE_STYLE[hovered.state].label}
                </motion.span>
              )}
            </AnimatePresence>
          </div>
...
function TajweedSection() {
  ...
  return (
    <section dir="rtl" className="relative bg-neutral-900 py-24 text-neutral-0 md:py-32">
      <SectionContainer>
        <SectionLede
          lead="التجويد"
        ...
        <div className="mt-14 grid grid-cols-1 gap-12 md:grid-cols-[1fr_1.2fr] md:items-center">
            ...
            {TAJWEED_LETTERS.map((item, i) => (
              <div key={item.letter} className="relative pb-3">
                <motion.button
                  ...
                  whileHover={{ y: -3 }}
                  whileTap={{ scale: 0.94 }}
                  animate={{ color: selected === i ? "var(--color-visual-teal)" : "var(--color-neutral-500)" }}
          ...
          <div className="border-r border-neutral-700 pr-8">
            <AnimatePresence mode="wait">
              ...
              >
                <p className="font-thmanyah-text text-caption uppercase tracking-[0.18em] text-visual-teal">{active.name}</p>
                <p className="mt-3 font-thmanyah-display text-h3 text-neutral-0">{active.makhraj}</p>
                <p className="mt-4 max-w-md font-thmanyah-text text-body leading-relaxed text-neutral-400">{active.description}</p>
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
...
function DailyPracticeSection() {
  return (
    <section dir="rtl" className="relative bg-neutral-50 py-24 md:py-32">
      <SectionContainer>
        <SectionLede lead="المتابعة" sub="وردك اليومي" body="لا حاجة لتذكّر ما بقي. الورد يعرض نفسه، جاهزًا كل صباح، ويتكيّف مع أدائك." />
        ...
        <div className="mt-16 grid grid-cols-1 gap-12 md:grid-cols-[1fr_320px]">
            ...
            {DAILY_REVIEW_ITEMS.map((item) => (
              <li key={item.label} className="flex items-center justify-between gap-4 py-5">
                <div className="flex items-center gap-4">
                  <VerseMarker index={item.status === "done" ? "✓" : "•"} filled={item.status === "done"} size="md" />
                  <span className="font-thmanyah-display text-h3-sm text-neutral-900">{item.label}</span>
                </div>
                <span className={`font-thmanyah-text text-caption ${item.status === "upcoming" ? "text-neutral-400" : "text-visual-teal"}`}>
                  {item.detail}
                </span>
              </li>
            ))}
          ...
          <div className="border-r-2 border-visual-teal/30 pr-6">
            <p className="font-thmanyah-text text-caption uppercase tracking-[0.18em] text-visual-teal">مراجعتك اليوم</p>
            ...
            <p className="mt-3 font-thmanyah-text text-caption text-neutral-500">
              أُخطئ فيها مرتين خلال الأسبوع الماضي — أُضيفت إلى ورد الغد. الرحلة تتكيّف معك، لا العكس.
            </p>
          </div>
        </div>
      </SectionContainer>
  ...
  return (
    <section dir="rtl" className="relative bg-neutral-0 py-24 md:py-32">
      ...
      <div className="relative left-1/2 mt-12 w-screen -translate-x-1/2">
        <div className="aspect-[16/7] w-full bg-cover bg-center" style={{ backgroundImage: "url('/backgrounds/bg-24.png')" }} />
      </div>
      ...
      <SectionContainer>
        ...
        <div className="mt-6 flex items-center gap-3">
          ...
          <div>
            <p className="font-thmanyah-text text-body text-neutral-900">{featured.name}</p>
            <p className="font-thmanyah-text text-caption text-neutral-500">{featured.focus} — {featured.years}</p>
          </div>
        </div>
        ...
        <ul className="mt-20 flex flex-col gap-8 md:flex-row md:gap-16">
          {rest.map((t) => (
            <li key={t.name}>
              <p className="font-thmanyah-display text-h3-sm text-neutral-900">{t.name}</p>
              <p className="mt-1 font-thmanyah-text text-caption text-neutral-500">{t.focus}</p>
              <p className="mt-1 font-thmanyah-text text-micro text-neutral-400">{t.years}</p>
            </li>
          ))}
        </ul>

        <div className="relative mt-16 overflow-hidden rounded-[6px] bg-neutral-900">
          <div className="aspect-video w-full bg-cover bg-center opacity-80" style={{ backgroundImage: "url('/backgrounds/bg-25.png')" }} />
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between rounded-[4px] border border-white/20 bg-black/40 px-4 py-3 backdrop-blur-md">
            <div className="flex items-center gap-3">
              ...
              <span className="font-thmanyah-text text-caption text-neutral-0">حلقة البقرة — مباشر الآن</span>
            </div>
            <span className="font-thmanyah-text text-micro text-neutral-300">{featured.name}</span>
          </div>
        </div>
      </SectionContainer>
    </section>
    ...
    <section dir="rtl" className="relative bg-neutral-50 py-20 md:py-24">
      <SectionContainer>
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 md:items-center">
          <div>
            <p className="font-thmanyah-text text-caption uppercase tracking-[0.18em] text-visual-teal">حلقة اليوم</p>
            <h3 className="mt-4 font-thmanyah-display text-h2-sm text-neutral-900">لست وحدك في المراجعة</h3>
          ...
          <ul className="flex flex-col gap-4 font-thmanyah-text text-body text-neutral-700">
            <li className="flex items-center gap-3"><span className="h-px w-8 bg-visual-teal" />١٢ طالبًا يراجعون الآن سورة البقرة</li>
            <li className="flex items-center gap-3"><span className="h-px w-8 bg-visual-teal" />حلقة المراجعة الجماعية — كل خميس، بعد المغرب</li>
            <li className="flex items-center gap-3"><span className="h-px w-8 bg-visual-teal" />يمكن الانضمام أو المتابعة الفردية دون التزام</li>
          </ul>
        </div>
      </SectionContainer>
    </section>
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ...
        { scaleY: 0 },
        {
          ...
          ease: "none",
          transformOrigin: "top",
          scrollTrigger: { trigger: rootRef.current, start: "top 70%", end: "bottom 75%", scrub: true },
        }
      );
    }, rootRef);
    return () => ctx.revert();
    ...
    <section ref={rootRef} dir="rtl" className="relative bg-neutral-0">
            ...
            <ul className="flex flex-col gap-12">
              {PROGRESS_MILESTONES.map((m) => (
                <li key={m.label} className="relative">
                  ...
                  <p className="font-thmanyah-text text-micro text-neutral-400">{m.date}</p>
                  <p className="mt-1 font-thmanyah-display text-h3-sm text-neutral-900">{m.label}</p>
                  <p className="mt-1 font-thmanyah-text text-caption text-neutral-500">{m.detail}</p>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="relative flex min-h-[60vh] items-center justify-center bg-neutral-50 py-28">
        ...
        <div className="relative mx-auto max-w-2xl px-6 text-center">
          ...
          <p className="mt-6 font-thmanyah-text text-caption text-neutral-500">سورة الإسراء، الآية ٨٢</p>
        </div>
      </div>
      ...
      <div className="relative bg-neutral-900 py-28 text-neutral-0 md:py-36">
        <SectionContainer>
          <div className="mx-auto max-w-xl text-center">
            <h3 className="font-thmanyah-display text-h2-sm text-neutral-0">ابدأ رحلتك مع القرآن</h3>
            <p className="mt-4 font-thmanyah-text text-body text-neutral-400">خطوة واحدة قد تغيّر علاقتك بالقرآن.</p>
            <div className="mt-10 flex justify-center">
              <Button href="/quran/ابدأ" variant="primary" size="lg">ابدأ الآن</Button>
            </div>
          </div>
        </SectionContainer>
      </div>
