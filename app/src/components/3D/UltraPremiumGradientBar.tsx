type UltraPremiumGradientBarProps = {
  className?: string;
  /**
...
/**
 * شريط ثلاثي الأبعاد متدرج الألوان — يعبر الحاوية الأب قطريًا من أقصى
 * اليمين إلى أقصى اليسار. التدرج يمر عبر ألوان المواد الأربعة (وردي،
 * برتقالي، بنفسجي أساسي، تركواز) فيقرأ كـ"شريط هوية" حقيقي بدل تدرج
 * بنفسجي-إلى-بنفسجي مسطّح. زخرفة مستقلة، بلا نص أو منطق داخلي، تُستخدم
 * خلف محتوى الهيرو أو أي قسم آخر يحتاج نفس اللمسة.
 *
 * الاستخدام: ضعه داخل حاوية أب لها `relative overflow-hidden`.
 */
export function UltraPremiumGradientBar({ className = "", offsetY = 0 }: UltraPremiumGradientBarProps) {
  return (
    <svg
      aria-hidden="true"
    ...
    >
      <defs>
        ...
        <linearGradient id="premiumBarShade" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="55%" stopColor="color-mix(in srgb, var(--color-primary) 40%, black)" stopOpacity="0" />
          <stop offset="100%" stopColor="color-mix(in srgb, var(--color-primary) 55%, black)" stopOpacity="0.28" />
        </linearGradient>
